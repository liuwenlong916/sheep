...
<template>
  <DTree :data="data">
    <template #expendIcon="{ toggleNode, treeNode, accordion }">
      <span v-if="treeNode.isLeaf" class="devui-tree-node__indent"></span>
      <span
        v-else
        @click="
          event => {
            event.stopPropagation()
            toggleNode(treeNode, accordion)
          }
        "
      >
        <svg
